...
const USER_PK = required("TEST_USER_PRIVATE_KEY");
const RECIPIENT = required("TEST_RECIPIENT_ADDRESS");
...
function required(name: string): string {
  ...
  if (!v || v.length === 0) {
    fail(`Missing required env var: ${name}. Set it before running this script.`);
  }
  return v as string;
}
...
async function main(): Promise<void> {
  ...
  console.log(`       tx: ${deposit.receipt.hash}\n`);
  ...
  console.log(`       tx: ${transfer.receipt.hash}\n`);
  ...
  console.log(`       tx: ${withdraw.receipt.hash}\n`);
  ...
  for (const [name, tx] of [
    ["deposit", deposit.receipt.hash],
    ["transfer", transfer.receipt.hash],
    ["withdraw", withdraw.receipt.hash],
  ] as const) {
